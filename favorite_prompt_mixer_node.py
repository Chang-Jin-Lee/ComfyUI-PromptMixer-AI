...
def _extract_header_and_body(text: str) -> Tuple[Tuple[str,int,float,str,str,str,Dict[str,float]], str]:
    """
    由ы꽩: ((name, steps, cfg, sampler, scheduler, checkpoint, loras_dict), body_text)
    """
...
class FavoritePromptMixerNode:
    ...
    (+Header에 name/steps/cfg/sampler/scheduler/checkpoint/LoRA 지정
     → MODEL/CLIP/VAE & positive_text/negative_text 동시 출력)
    - positive/negative는 TEXT로만 출력됩니다.
    - CLIP 인코딩은 별도의 'CLIP Text Encode' 노드에서 수행하세요.
    """
...
"""
    ...
    def build(self, prompts_text, always_prefix, always_suffix, negative_text,
        ...
        # (5) 체크포인트 로드 + LoRA 적용 (→ model/clip/vae 출력)
        ...
        # (6) 출력: name, steps, cfg, sampler, scheduler, MODEL, CLIP, VAE, positive_text, negative_text
        return (
            str(name_val or ""),
            int(steps_val),
            float(cfg_val),
